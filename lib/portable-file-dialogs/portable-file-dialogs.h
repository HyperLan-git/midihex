...
inline std::string path::home() {
    ...
    if (OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, &token)) {
        ...
        dll::proc<BOOL WINAPI(HANDLE, LPSTR, LPDWORD)>
            get_user_profile_directory(userenv, "GetUserProfileDirectoryA");
    ...
    if (m_future.valid()) {
        ...
        for (auto hwnd : m_windows)
            if (previous_windows.find(hwnd) == previous_windows.end()) {
                ...
                // Also send IDNO in case of a Yes/No or Abort/Retry/Ignore
                // messagebox
...
#if _WIN32
inline void internal::executor::start_func(
    std::function<std::string(int *)> const &fun) {
    stop();
...
inline void internal::executor::start_process(
    ...
    if (m_pid == 0) {
        ...
        // Ignore stderr so that it doesn’t pollute the console (e.g. GTK+
        // errors from zenity)
...
inline std::string internal::dialog::osascript_quote(
    std::string const &str) const {
    return "\"" + std::regex_replace(str, std::regex("[\\\\\"]"), "\\$&") +
           "\"";
}
...
inline internal::file_dialog::file_dialog(
    ...
    for (size_t i = 0; i + 1 < filters.size(); i += 2) {
        ...
        filter_list +=
            std::regex_replace(filters[i + 1], whitespace, ";") + '\0';
    }
    filter_list += '\0';
    ...
    m_async->start_func([this, in_type, title, default_path, filter_list,
        ...
        if (in_type == type::folder) {
            ...
            if (flags(flag::is_vista)) {
                // On Vista and higher we should be able to use IFileDialog for
                // folder selection
                IFileDialog *ifd;
                HRESULT hr = dll::proc<HRESULT WINAPI(REFCLSID, LPUNKNOWN,
                                                      DWORD, REFIID, LPVOID *)>(
                    ole32, "CoCreateInstance")(CLSID_FileOpenDialog, nullptr,
                                               CLSCTX_INPROC_SERVER,
                                               IID_PPV_ARGS(&ifd));

                // In case CoCreateInstance fails (which it should not), try
                // legacy approach
                if (SUCCEEDED(hr))
    ...
    if (is_osascript()) {
        ...
        if (in_type == type::open) {
            ...
            std::sregex_token_iterator iter(patterns.begin(), patterns.end(),
                                            sep, -1);
            ...
            if (has_filter && filter_list.size() > 0) {
                // There is a weird AppleScript bug where file extensions of
                // length != 3 are ignored, e.g. type{"txt"} works, but
                // type{"json"} does not. Fortunately if the whole list starts
                // with a 3-character extension, everything works again. We use
                // "///" for such an extension because we are sure it cannot
                // appear in an actual filename.
        ...
        // If the default path is a directory, make sure it ends with "/"
        // otherwise zenity will open the file dialog in the parent directory.
...
inline std::string internal::file_dialog::select_folder_vista(IFileDialog *ifd,
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            // This is unlikely to fail because we use FOS_FORCEFILESYSTEM, but
            // try to output a debug message just in case.
            if (SUCCEEDED(item->GetDisplayName(SIGDN_FILESYSPATH, &wname))) {
                result = internal::wstr2str(std::wstring(wname));
                dll::proc<void WINAPI(LPVOID)>(ole32_dll(),
                                               "CoTaskMemFree")(wname);
            } else {
                if (SUCCEEDED(
                        item->GetDisplayName(SIGDN_NORMALDISPLAY, &wname))) {
                    auto name = internal::wstr2str(std::wstring(wname));
                    dll::proc<void WINAPI(LPVOID)>(ole32_dll(),
                                                   "CoTaskMemFree")(wname);
                    std::cerr << "pfd: failed to get path for " << name
                              << std::endl;
                } else
                    std::cerr << "pfd: item of unknown type selected"
                              << std::endl;
            }
...
inline message::message(std::string const &title, std::string const &text,
    ...
    m_async->start_func([text, title, style](int *exit_code) -> std::string {
        ...
        new_style_context ctx;
        *exit_code = MessageBoxW(GetActiveWindow(), wtext.c_str(),
                                 wtitle.c_str(), style);
        return "";
    });
...
#undef PFD_OSX_ICON
        ...
        switch (_choice) {
            case choice::ok_cancel:
                ...
                break;
            case choice::yes_no:
                // Do not use standard --question because it causes “No” to
                // return -1, which is inconsistent with the “Yes/No/Cancel”
                // mode below.
